    ...
    )
    {
        ...
        if (isset($date_working_plan['breaks']))
        {
            ...
            foreach ($date_working_plan['breaks'] as $index => $break)
            {
                ...
                {
                    $period_start_record = new DateTime($period['start']);
                    $period_end_record = new DateTime($period['end']);
                    $period_start = $roundToHalfHour($period_start_record, FALSE);
                    $period_end = $roundToHalfHour($period_end_record, TRUE);
        ...
        foreach ($appointments as $appointment)
        {
            ...
            {
                $appointment_start_record = new DateTime($appointment['start_datetime']);
                $appointment_end_record = new DateTime($appointment['end_datetime']);
                $appointment_start = $roundToHalfHour($appointment_start_record, TRUE);
                $appointment_end = $roundToHalfHour($appointment_end_record, FALSE);
